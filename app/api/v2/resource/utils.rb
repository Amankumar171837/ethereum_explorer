  ...
  module Resource
    module Utils
      ...
      def update_user(user)
        Barong::Management::User.new.update_user({ user: user.as_payload.to_json })
        Rails.logger.info "User updated with UID: #{user.uid}"
      end

      def update_email_notifications(user)
        user.email_notifications.where('email != ? && enabled = false', user.email)
