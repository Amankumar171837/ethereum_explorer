    ...
    class Phones < Grape::API
      ...
      resource :phones do
        ...
        post do
          ...
          if current_user.phones.length >= Barong::App.config.user_phones_limit.to_i && phone.nil?
            error!({ errors: ['phone_number.limit.exceeded'] }, 422)
          end

          if Phone.where_number_is(phone_number).pluck(:validated_at).any?
            error!({ errors: ['resource.phone.exists'] }, 400)
          end

          phone ||= current_user.phones.create!(number: phone_number)
          code_error!(phone.errors.details, 422) if phone.errors.any?

          if phone.time_before_resend && Time.now < phone.time_before_resend
            error!({ errors: ['resource.code.request_after.resend_time'] }, 404)
          end
          phone.set_code
          send_confirmation(phone, declared_params[:channel])
        ...
        post '/send_code' do
          ...
          if Phone.where_number_is(phone_number).pluck(:validated_at).any? || phone.nil?
            error!({ errors: ['resource.phone.doesnt_exist'] }, 400)
          end
        ...
        post '/verify' do
          ...
          if Phone.where_number_is(phone_number).pluck(:validated_at).any? || phone.nil?
            error!({ errors: ['resource.phone.doesnt_exist'] }, 404)
          end

          error!({ errors: ['resource.phone.code_is_expired'] }, 422) unless phone.code_expiry_date >= Time.now

          verification = PlatformSetting.verify_service(phone_number)
                                        .verify_code?(number: phone_number,
                                                      code: declared_params[:verification_code],
                                                      user: current_user)

          error!({ errors: ['resource.phone.verification_invalid'] }, 404) unless verification

          phone.update(validated_at: Time.current, code_expiry_date: 1.minute.ago(Time.now))
