        ...
        resource :activities do
          ...
          helpers do
            def permitted_search_params(params)
              ...
              params.slice(:action, :uid, :email, :topic, :from, :to, :range, :target_uid,
                           :country, :country_code).merge(with_user: true, ordered: true)
            end
          end
          ...
          desc 'Returns array of Activities as paginated collection from Influxdb'
          params do
            use :activity_attributes
            use :timeperiod_filters
            use :pagination_filters
          ...
          get 'data' do
            ...
            params[:user_id] = User.find_by_uid(params[:uid]).id if params[:uid].present?
            params[:user_email] = params[:email] if params[:email].present?
            activities = Activity.get_influx_data(params.except(:uid, :email, :page, :limit))
            present paginate(activities), with: API::V2::Admin::Entities::ActivityWithInflux
          end
        end
      end
    end
