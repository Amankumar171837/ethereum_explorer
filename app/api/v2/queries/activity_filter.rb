...
module API::V2::Queries
  class ActivityFilter
    ...
    def filter_by_action(scoped, action = nil)
      action ? scoped.where(activities: { action: action.split(', ') }) : scoped
    end
    ...
    def filter_by_country(scoped, country = nil)
      country ? scoped.where(activities: { country: country }) : scoped
    end

    def filter_by_country_code(scoped, country_code = nil)
      country_code ? scoped.where(activities: { country_code: country_code }) : scoped
    end
  end
end
