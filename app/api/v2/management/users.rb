  ...
  module Management
    class Users < Grape::API
      ...
      resource :users do
        ...
        desc 'Getting device tokens for array of users' do
          @settings[:scope] = :read_users
        end
        params do
          requires :uids,
        ...
        post '/device-tokens' do
          tokens = {}
          users = User.where(uid: params[:uids])
          users.each do |user|
            active_device = user.devices.active
            tokens[user.uid] = active_device.pluck(:device_token).compact
          end
