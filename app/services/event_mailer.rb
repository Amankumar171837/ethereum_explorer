...
class EventMailer
  ...
  def handle_message(delivery_info, _metadata, payload)
    ...
    # Update user object if asked to use *latest_email* address
    user.email = user.latest_email if obj.record.latest_email

    notification = user.email_notification(config[:type])

    unless notification&.enabled?
      @bunny_channel.ack(delivery_info.delivery_tag)
      Rails.logger.info "Skipped email notification for user #{user.email}."
      return
    end
    ...
    if obj.record.from.present?
      FullPageMailer.process_payload(params).deliver_now
    else
  ...
  def skip_event(event, expression)
    ...
    res = values.keys.map do |field_name|
      safe_dig(event, field_name.to_s.split('.')).in?(values[field_name])
    end
