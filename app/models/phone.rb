...
class Phone < ApplicationRecord
  ...
  # ::TODO: Remove me
  # Now we have moved the code generation to *set_code* method
  ...
  store_accessor :metadata, %i[time_before_resend code_expiry_date]
  ...
  class << self
    ...
    def international(unsafe_phone)
      self.sanitize parse(unsafe_phone).international(false)
    end
    ...
    def where_number_is(number, attrs={})
      attrs.merge!(number_index: SaltedCrc32.generate_hash(number))
      where(attrs)
    end
  end
  ...
  # ::TODO: Remove me
  # Code generation moved to *set_code* method.
  def generate_code
