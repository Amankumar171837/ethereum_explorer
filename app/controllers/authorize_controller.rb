...
class AuthorizeController < ActionController::Metal
  ...
  # expired or deleted authentication token
  rescue_from JWTSessions::Errors::Expired do |e|
    render json: [{ message: e.message, code: 403 }], status: :forbidden
  end
  ...
  def set_location_headers
    location = Barong::GeoIP.info(ip: remote_ip)
    response.headers.merge!({ 'X-User-Ip' => remote_ip,
                              'X-Country-Code' => location[:country_code],
                              'X-Continent' => location[:continent],
                              'X-City' => location[:city],
                              'X-Longitude' => location[:longitude],
                              'X-Latitude' => location[:latitude] }.compact)
  end
  ...
  def first_matched_restriction(category)
    ...
    location = Barong::GeoIP.info(ip: request_ip, keys: %i[country continent])
    ...
    if restriction = @restrictions[category]['continent']&.find { |r| r[0]&.casecmp?(location[:continent]) }    then return restriction end
    if restriction = @restrictions[category]['country']&.find { |r| r[0]&.casecmp?(location[:country]) }        then return restriction end
  end
