...
module Barong
  ...
  class Authorize
    ...
    def auth
      ...
      auth_type = 'bearer_token' if bearer_token_header?
    ...
    def cookie_owner
      ...
      error!({ errors: ['authz.invalid_session'] }, 401) unless user
      ...
      unless user.state.in?(%w[active pending])
        error!({ errors: ['authz.user_not_active'] }, 403)
      end
    ...
    def bearer_token_owner
      token_value = Barong::OpaqueJwt::Token.get_bearer_token(headers['Authorization'])

      begin
        decoded_token = authenticator.decode(token_value)
      rescue => e
        Rails.logger.warn("JWT decoding failed: #{e.message}")
        error!({ errors: ['authz.invalid_session'] }, 401)
      end

      unless decoded_token.has_key?(:uuid) && decoded_token[:uid].present?
        error!({ errors: ['authz.invalid_session'] }, 401)
      end

      jwt_session.send(:access_token_data, token_value)

      user = User.find_by!(uid: decoded_token[:uid])
      error!({ errors: ['authz.invalid_session'] }, 401) unless user

      Rails.logger.debug "User #{user} authorization via authorization"

      unless user.state.in?(%w[active pending])
        JWTSessions::Session.new.flush_by_uuid(decoded_token[:ruuid])
        error!({ errors: ['authz.invalid_session'] }, 401)
      end

      validate_permissions!(user)
      user # returns user(whose session is inside authorization bearer token)
    end
    ...
    def remote_country_code
      return @request.headers['Cf-Ipcountry'] if @request.headers['Cf-Ipcountry'].present?

      Barong::GeoIP.info(ip: remote_ip, keys: [:country_code])[:country_code]
    end
    ...
    def bearer_token_header?
      ...
      error!({ errors: ['authz.invalid_authorization_headers'] }, 422) if headers['Authorization'].blank?
